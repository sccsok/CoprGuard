...
class BeatGANsUNetModel(nn.Module):
    ...
    def forward(self, x, t, y=None, **kwargs):
        ...
        if self.conf.num_classes is not None and y is not None:
            ...
            drop_mask = (th.rand(b, device=label_embs.device) < 0.1).float().view(-1, 1)
            unconditional_emb = th.zeros_like(label_embs) 
            label_embs = th.where(drop_mask.bool(), unconditional_emb, label_embs)
            emb = emb + label_embs
