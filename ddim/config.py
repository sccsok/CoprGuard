...
class TrainConfig(BaseConfig):
    ...
    eval_path: str = None
    base_dir: str = os.path.expanduser('~/CoprGuard/checkpoints')
    use_cache_dataset: bool = False
    data_cache_dir: str = os.path.expanduser('~/cache')
    work_cache_dir: str = os.path.expanduser('~/CoprGuard/ddim')
    # to be overridden
    name: str = ''
    # dataset
    data_dir = os.path.expanduser("~/CoprGuard")
    ...
    def fid_cache(self):
        ...
        data_name = self.name.split('_')[0][:-3]
        self._fid_cache = f'{self.work_cache_dir}/eval_images/{data_name}_size{self.img_size}_{self.eval_num_images}'
        return self._fid_cache

    @fid_cache.setter
    def fid_cache(self, input_dir):
        self._fid_cache = input_dir
    ...
    def generate_dir(self):
        # we try to use the local dirs to reduce the load over network drives
        # hopefully, this would reduce the disconnection problems with sshfs
        return f'{self.work_cache_dir}/gen_images/{self.name}_1000'
    ...
    def make_dataset(self, path=None, **kwargs):
        data_names = {
            'ffhq': 'Source/FFHQ', 
            'lsun': 'Source/LSUN', 
            'biggan': 'Source/BigGAN', 
            'progan': 'Source/ProGAN', 
            'stylegan': 'Source/StyleGAN', 
            'stylegan2': 'Source/StyleGAN2',
            'hinet': 'HiNet/FFHQ_041388',   # "HiNet_Lena"
            'celeb': 'Source/CelebA-HQ',
            'diagnosis': 'DIAGNOSIS/FFHQ',
            'vgg': 'Source/Vggface'
            }
        if 'merge' in self.name:
            strings = self.name.split('_')
            method = []
            method.extend([data_names[m] for m in [strings[0], strings[1]]])
            if int(strings[3]) is not 0:
                mix = 1.0 - float(strings[3]) / float(strings[4])
            else:
                mix = float(strings[4]) / 10.
        else:
            method = data_names[self.name.split('_')[0][:-3]]
            mix = 1.0
        
        return ALL_Dataset(method=method, mix=mix, data_dir=self.data_dir)
    
        """
        # For Classifier-Free training on ImageNet
        from torchvision import transforms
        data_dir = f'{self.data_dir}/Source/FFHQ'
        transform = [
