...
def plot_prnu(
    ...
    fig = plt.figure(
        figsize=(
            width,
            width * 1.5 / ((len(labels) // 2) if fixed_height is None else fixed_height) + 0.1,
        )
    )
    grid = AxesGrid(
        fig, 111, nrows_ncols=(2, len(labels) // 2), cbar_mode="edge", cbar_size="20%"
    )
    for ax in grid:
        ax.axes.xaxis.set_ticks([])
        ax.axes.yaxis.set_ticks([])
    ...
    for i, (array, label) in enumerate(zip(data, labels)):
        ...
        if len(data) != 1:
            grid[i].set_title(label, pad=2, fontsize = 5)

        if i == len(labels) - 1 or i == len(labels) // 2 - 1:
