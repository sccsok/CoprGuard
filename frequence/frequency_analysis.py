...
from src.image import dwt, dct, fft, spectral_density
...
TRANSFORMS = {
    "dwt_ca": cytoolz.compose_left(
        partial(dwt, mode="ca"), get_mean_std
    ),
    "dwt_ch": cytoolz.compose_left(
        partial(dwt, mode="ch"), get_mean_std
    ),
    "dwt_cv": cytoolz.compose_left(
        partial(dwt, mode="cv"), get_mean_std
    ),
    "dwt_cd": cytoolz.compose_left(
        partial(dwt, mode="cd"), get_mean_std
    ),
...
def main(
    ...
    feature_dir = output_dir / "features" / transform
    ...
    for key in np.abs(df["mean"]).keys():
        np.save(f"{feature_dir}/{key}.np", df["mean"][key])
...
def parse_args():
    ...
    parser.add_argument(
        "--num-workers", type=int, default=1, help="Number of workers (default: 4)."
    )
    parser.add_argument(
        "--experiment",
        default="default",
