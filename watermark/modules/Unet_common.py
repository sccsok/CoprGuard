...
def mask_high_freq(coeffs, ratio=0.7):
    ...
    return LL, LH_d, HL_d, HH_d
    
def dwt_init(x, clip_high=False, ratio=0.5, flag=False):
    ...
    if clip_high:
        x_LL, x_HL, x_LH, x_HH = mask_high_freq([[x_LL, x_HL, x_LH, x_HH]], ratio=ratio)

    if flag:
        return torch.cat((x_LL, x_HL * 0.0, x_LH * 0.0, x_HH * 0.0), 1)
    else:
        return torch.cat((x_LL, x_HL, x_LH, x_HH), 1)

def iwt_init(x, flag=False):
    r = 2
...
class DWT(nn.Module):
    ...
    def forward(self, x, clip_high=False, ratio=0.5, flag=False):
        return dwt_init(x, clip_high, ratio, flag=flag)
